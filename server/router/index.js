...
module.exports = () => {
  ...
  route.get('/cart', (req, res) => {
    const cartStr =
      'SELECT cart_id,user.user_id,product.product_id,product_name,product_uprice,product_img_url,goods_num,product_num,shop_name FROM product,user,goods_cart,shop where product.product_id=goods_cart.product_id and user.user_id=goods_cart.user_id and shop.shop_id = product.shop_id'
  ...
  route.get('/register', (req, res) => {
    const regName = req.query.username
    let regPasswd = req.query.passowrd
  ...
  route.get('/login', (req, res) => {
    ...
    const bPassword = common.md5(password + common.MD5_SUFFXIE)
    ...
    db.query(selectUser, (err, data) => {
      if (err) {
        ...
        if (data.length === 0) {
          ...
          const dataw = data[0]
          // login sucess
          if (dataw.login_password === bPassword) {
            // save the session
